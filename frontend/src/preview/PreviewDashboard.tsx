/**
 * /preview — Dashboard redesign prototype
 *
 * NON-DESTRUCTIVE: this page is entirely self-contained. It imports its own
 * Tailwind CSS (preview.css) and shadcn/ui-style components from ./ui/.
 * Tailwind preflight is disabled in tailwind.config.js so nothing here
 * bleeds into the existing hand-rolled pages.
 *
 * Real data: useAuth() for the user, GET /api/games for active games.
 * Mock data: stats strip (clearly labelled "Sample data").
 */
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        'hover:shadow-md hover:border-indigo-200/80 dark:hover:border-indigo-700/60',
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div
            className={cn(
              ...
              'border border-indigo-100/80 dark:border-indigo-800/60',
            )}
          >
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1 mt-1.5">
              ...
              {players.length > 5 && (
                <span className="text-[9px] text-slate-400 ml-0.5">+{players.length - 5}</span>
              )}
            </div>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      {[0, 1, 2].map((i) => (
        <div
          key={i}
          className="p-4 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 bg-white/80 dark:bg-slate-800/80"
        >
...
interface TopBarProps {
  user: { nickname: string; themePref: string } | null;
  onThemeToggle: () => void;
}

function TopBar({ user, onThemeToggle }: TopBarProps) {
  ...
  return (
    ...
    >
      {/* Backdrop with scroll-driven opacity */}
      <motion.div
        className="absolute inset-0 backdrop-blur-md border-b border-slate-200/60 dark:border-slate-700/60 bg-white/80 dark:bg-slate-900/80"
      ...
      <div className="relative flex items-center gap-3">
        ...
        >
          {user?.themePref === 'dark' ? <Sun size={15} /> : <Moon size={15} />}
        </button>
        {user && (
...
export function PreviewDashboard() {
  ...
  // Keep root element's data-theme in sync with app theme so Tailwind dark:
  // variants fire correctly inside the scoped root.
  useEffect(() => {
    const observer = new MutationObserver(() => {
      const docTheme = document.documentElement.getAttribute('data-theme') ?? '';
      if (rootRef.current) {
        rootRef.current.setAttribute('data-theme', docTheme);
      }
    });
    ...
    rootRef.current?.setAttribute('data-theme', current);
  ...
  // Filter logic (demonstrative — "friends" filter is mock since we don't have that data)
  ...
  return (
    <div
      ref={rootRef}
      className="preview-root min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-200"
    >
      {/* Subtle background gradient */}
      <div className="fixed inset-0 pointer-events-none gradient-mesh" aria-hidden />

      {/* Top bar */}
      <TopBar user={user} onThemeToggle={handleThemeToggle} />

      {/* Main content */}
      <main className="relative pt-20 pb-16 px-4 sm:px-6 max-w-2xl mx-auto">
        <motion.div variants={containerVariants} {...motionConfig} className="flex flex-col gap-6">
          {/* Welcome header */}
          ...
          <motion.div variants={itemVariants}>
            ...
            {/* Segmented filter — only show when there are games */}
            {!loading && activeGames.length > 0 && (
              <motion.div
