...
- **release-prep-and-cut** — no semver release process yet (no releases, no CHANGELOG roll, no
  published images). Adoption is planned with the release pipeline (next prompt after public-repo
  prep); the standard composes on top of `code-checkin-and-pr`.
